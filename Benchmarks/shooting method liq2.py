...
is_liquid_m0 = np.zeros(N, dtype=bool)
is_liquid_m1 = np.zeros(N, dtype=bool)
is_liquid_m2 = np.zeros(N, dtype=bool)

is_liquid_m1[radius_array <= R_core_m1] = True
is_liquid_m2[radius_array <= R_core_m2] = True
...
def liq_func(x, mode):
    ...
    if mode == 0:
        ...
        is_liq = np.zeros(x.shape, dtype=bool)
        is_liq[x <= R_core_m1] = True
        return is_liq
    elif mode == 2:
        is_liq = np.zeros(x.shape, dtype=bool)
        is_liq[x <= R_core_m2] = True
        return is_liq
...
interp_funcs = {
    ...
    'm2': (lambda x: np.interp(x, radius_array, complex_shear_array_m2),
           ...
           lambda x: liq_func(x, mode=2))
}

def deriv_m0(x, y):
    shear_modulus, bulk_modulus, density, gravity, is_liq_func = interp_funcs['m0']

    result = radial_func_derivatives_general_liq(x, y, shear_modulus(x), bulk_modulus(x), density(x), gravity(x), orbital_freq, is_liq_func(x))

    return result

def deriv_m1(x, y):
    shear_modulus, bulk_modulus, density, gravity, is_liq_func = interp_funcs['m1']
...
def deriv_m2(x, y):
    shear_modulus, bulk_modulus, density, gravity, is_liq_func = interp_funcs['m2']
...
def bc_residual(ya, yb):
    ...
    return (ya[0], yb[1], ya[2], yb[3], ya[4], yb[5] - surf_condition, yb[6])
...
for structure_name, radii in solved_radii_by_model.items():
    ...
    y2 = np.zeros_like(radii)
    y3 = np.ones_like(radii)
    y4 = np.zeros_like(radii)
    y5 = np.ones_like(radii)
    y6 = np.ones_like(radii)
    y7 = np.zeros_like(radii)

    guess = np.vstack((y1, y2, y3, y4, y5, y6, y7))
