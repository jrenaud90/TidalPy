...
from TidalPy.RadialSolver_x.interfaces.interfaces import solve_upper_y_at_interface
...
def test_interface_accuracy(lower_layer_type, lower_is_static, upper_layer_type, upper_is_static,
                            lower_is_incompressible, upper_is_incompressible):
    """Use results from TidalPy 0.4.0 to test accuracy of the RadialSolver_x interface solver."""
    ...
    if (lower_layer_is_solid, lower_is_static, upper_layer_is_solid, upper_is_static) not in tpy_0p4_results:
        pytest.skip(f'Combination not found in pre-calculated TidalPy v0.4 results.')
        ...
        assert np.allclose(upper_y_no_nan.flatten(), comparison_results_no_nan)
