...
def test_radial_solver_1layer(layer_type, is_static, is_incompressible, method, degree_l,
    ...
    # TODO: The solve is numerically unstable for 1-layer planets that are entirely liquid; skip those.
    if layer_type != 'solid':
...
def test_radial_solver_1layer_solve_for_both(layer_type, is_static, is_incompressible,
    ...
    if layer_type != 'solid':
        ...
        try:
            ...
            # API compatibility: some builds expose Q/lag aliases, others expose Q_k/lag_k only.
            quality = out.Q if hasattr(out, 'Q') else out.Q_k
            lag = out.lag if hasattr(out, 'lag') else out.lag_k
            assert type(quality) is np.ndarray
            assert quality.shape == (len(solve_for),)
            assert type(lag) is np.ndarray
            assert lag.shape == (len(solve_for),)
