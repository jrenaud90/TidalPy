...
def test_unknown_and_reserved_methods():
    ...
    with pytest.raises(ValueError, match="unknown Love-number method"):
        world.solve_love_numbers(frequency_rad_s=FREQ, love_method="shoot")
    with pytest.raises(NotImplementedError, match="laterally_inhomogeneous"):
        world.solve_love_numbers(frequency_rad_s=FREQ, love_method="3d")
    with pytest.raises(ValueError, match="only the radial_solver"):
        world.solve_love_numbers_supplied(
            np.full(5, SHEAR + 0j), np.full(5, BULK + 0j), np.linspace(0.0, RADIUS, 5), frequency_rad_s=FREQ,
...
def test_volume_average_excludes_non_tidal_layers():
    ...
    result_in = included.solve_love_numbers(frequency_rad_s=FREQ, love_method="homogeneous")
    result_ex = excluded.solve_love_numbers(frequency_rad_s=FREQ, love_method="homogeneous")
...
def test_cpl_and_ctl_structure():
    ...
    static = world.solve_love_numbers(frequency_rad_s=FREQ, love_method="homogeneous")
    assert static["love_number_k"].imag == 0.0
    cpl = world.solve_love_numbers(frequency_rad_s=FREQ, love_method="cpl", fixed_q=50.0)
    ...
    ctl = world.solve_love_numbers(frequency_rad_s=FREQ, love_method="ctl", fixed_dt=600.0)
...
def test_cpl_uses_static_modulus():
    ...
    viscoelastic = world.solve_love_numbers(frequency_rad_s=FREQ, love_method="homogeneous")
    assert abs(viscoelastic["love_number_k"].imag) > 1e-3
    cpl = world.solve_love_numbers(frequency_rad_s=FREQ, love_method="cpl", fixed_q=100.0)
...
def test_cpl_ctl_parameters_from_tide_model():
    ...
    with pytest.raises(ValueError, match="fixed_q"):
        world.solve_love_numbers(frequency_rad_s=FREQ, love_method="cpl")
    with pytest.raises(ValueError, match="fixed_dt"):
        world.solve_love_numbers(frequency_rad_s=FREQ, love_method="ctl")
    world.set_tide_model(make_tide("cpl", {"fixed_k": [0.3], "fixed_q": [40.0]}))
    from_model = world.solve_love_numbers(frequency_rad_s=FREQ, love_method="cpl")
    explicit = world.solve_love_numbers(frequency_rad_s=FREQ, love_method="cpl", fixed_q=40.0)
    assert from_model["love_number_k"] == explicit["love_number_k"]
    with pytest.raises(ValueError, match="degree 3"):
        world.solve_love_numbers(frequency_rad_s=FREQ, degree_l=3, love_method="cpl")   # no Q_3 on the model
    world.set_tide_model(make_tide("ctl", {"fixed_k": [0.3], "fixed_dt": [300.0]}))
    from_model = world.solve_love_numbers(frequency_rad_s=FREQ, love_method="ctl")
    assert from_model["love_number_k"] == world.solve_love_numbers(
        frequency_rad_s=FREQ, love_method="ctl", fixed_dt=300.0)["love_number_k"]
...
def test_build_world_round_trips_love_method():
    ...
    rebuilt.solve_eos()
    result = rebuilt.solve_love_numbers(frequency_rad_s=FREQ, love_method="cpl")
