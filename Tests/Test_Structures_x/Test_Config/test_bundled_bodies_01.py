...
Io additionally has its asthenosphere viscosity fitted so that the total tidal dissipation matches the
astrometric value of Lainey et al. (2009), and its per-layer `tidal_scale` values come from integrating
the depth-resolved 3D heating. Both are pinned here, because both are claims the file makes in its
comments and either could rot silently: a rheology default changing under it would move the heat output,
and scales that stop summing to one would double count the heat.
...
class Body:
    ...
    def __init__(self, name, radius, mass, gravity, layers, tidal_layers, spin_period_days,
...
_BODIES = [
    Body("io", 1821490.0, 8.9298e22, 1.796,
         ...
         1.769, semi_major_axis=4.217e8, eccentricity=0.0041),
...
@pytest.mark.parametrize("body", _cases())
def test_bundled_body_spins_synchronously_at_its_classic_period(body):
    """Spin came from the classic file's spin_period; both bodies are synchronous rotators."""
    world = build_world(body.name)
