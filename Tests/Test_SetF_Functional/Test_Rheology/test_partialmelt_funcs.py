...
def test_all_models():
    ...
    assert type(off_visco) is float
    assert type(off_shear) is float
    ...
    assert type(spohn_visco) is float
    assert type(spohn_shear) is float
    ...
    henn_visco, henn_shear = henning(melt_fraction, temperature, premelt_viscosity, liquid_viscosity, premelt_shear,
                                     *henning_input)
    assert type(henn_visco) is float
    assert type(henn_shear) is float
