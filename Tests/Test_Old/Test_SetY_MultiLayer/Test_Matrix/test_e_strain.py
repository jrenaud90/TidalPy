...
radius_array = np.linspace(0., 1.e6, 10)
...
upper_radius_bylayer_array = (radius_array[-1])
mass_below = np.asarray([np.sum(mass_array[:i + 1]) for i in range(10)])
complex_shear_modulus_array = 5.e10 * np.ones(10, dtype=np.complex128)
complex_bulk_modulus_array = 10.e10 * np.ones(10, dtype=np.complex128)
...
def test_calc_strains_simple():
    ...
    strain_components, stress_components = \
        calculate_strain_stress(
                ...
                radius_array=radius_array[1:], shear_moduli=radial_solution.shear_modulus_array,
                bulk_moduli=radial_solution.bulk_modulus_array, frequency=orbital_freq, order_l=2
                )
