...
# Worst-case error amplification of the shooting method's surface boundary condition solve. Values near 1 are well
# conditioned; the achievable relative accuracy of the Love numbers is about this value times machine epsilon.
# Recorded only when the solve runs with `warnings=True`; it stays 0 otherwise and for the propagation matrix method.
rs_solution.surface_solve_amplification
