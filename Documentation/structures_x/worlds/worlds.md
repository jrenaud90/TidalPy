...
A world takes one of its heavy calls at a time: two threads sharing a world take turns on `solve_eos`, the Love solves, `calc_tides`, the 3D calls, `release_radial_solution`, and `load_binary`, while separate worlds run in parallel (these calls release the GIL). Do not change a world's layers or models from one thread while another thread uses it.
