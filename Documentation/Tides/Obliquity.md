...
The wider literature calls these inclination functions. For tides the angle that matters is between the deformed body's spin axis and its orbital plane about its host, normally called its obliquity. Several modes stay non-zero at zero obliquity, which is why a dedicated zero-obliquity form exists rather than simply passing zero.
...
New development happens in the C++ backend, whose equivalent is documented in [Obliquity Functions](../Tides_x/obliquity.md). That page covers the truncation levels, their mode counts, and the behavior of the functions for an aligned body.
