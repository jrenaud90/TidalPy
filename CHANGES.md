...
* `TidalPy.Tides_x`: The tide entry points (`set_tide_config`, `collapse_global_tides`, `global_potential`, `tidal_potential_3d_modes`, `eccentricity_func`) now validate the eccentricity truncation up front with a clear message listing the tabulated levels (1..5, 10, 15, 20); an untabulated level previously surfaced as an opaque "global potential failed" error at solve time. The obliquity truncation gets the same treatment: every entry point rejects an untabulated level (supported: 0/off, 2, 4, 10/general) with a message listing the options (`set_tide_config` and `tidal_potential_3d_modes` previously did not check it at all), and the world builder promotes an untabulated configured level to the next tabulated one with a once-per-session warning (anything above 4 promotes to the exact general form), mirroring the eccentricity promotion. Tested in `Tests/Test_Tides_x/Test_Potential/test_truncation_validation_01.py`. `eccentricity_func` also no longer raises on its own default argument (the old default `'gen'` had no string mapping; the default is now the tabulated level 3), and the drivers' error messages name the real supported sets. `calc_love_numbers`/`calc_neg_imk` type their optional `solver_love` argument as `LoveNumbers` (an arbitrary object was previously cast unchecked, which could crash the process).
...
* The propagation matrix method now has numerical accuracy tests (`Tests/Test_RadialSolver_x/Test_PropMatrix/test_c_matrix_parity.py`; it previously had only success/shape assertions): the `_x` matrix solver reproduces the closed-form homogeneous incompressible Love numbers to machine precision with core model 0 (and to ~1e-5 with the alternate core models 1..3, including the exact `h/k = (2l+1)/3` ratio), matches the original solver's matrix method to ~1e-10, and agrees with the shooting method at the inertial-term level.
* The 3D tidal potential engine's mode amplitudes are now validated against an independent reference (`Tests/Test_Tides_x/Test_Potential/test_potential_3d_01.py`): amplitude ratios between eccentricity sidebands equal Kaula's eccentricity-function ratios from the literature series (convention-free, matching to ~1e-7), the colatitude dependence follows `P_22` exactly, and amplitudes converge with rising eccentricity truncation. New obliquity collapse tests (`Tests/Test_Structures_x/Test_Worlds/test_world_3d_tides_obliquity_01.py`) verify the 1D and fully collapsed 3D heating agree with a nonzero obliquity at every supported obliquity truncation, that the heating converges as the truncation rises, and that the truncation-0 setting ignores obliquity entirely. Also new: unit tests for the point `volumetric_heating` binding and the `constant_time_lag_and_q` tide-model alias.
* Demos and benchmarks polish pass: demos 06/08/12 and the performance suite now use the new `homogeneous_love_numbers` helper instead of four hand-rolled copies of the homogeneous-grid radial solve, and demo 12 imports the new `year` constant instead of hard-coding it. The performance suite gained five tasks (a solid / static-liquid / solid 3-layer shooting solve, a homogeneous propagation-matrix solve, a fully collapsed 3D rheology-tide heating total, a sol-system binary round trip, and a Birch-Murnaghan `solve_eos`). Demo 09 now states the expected grid-level agreement (~5%) for its 1D-vs-3D consistency check, demos 09/10 explain when to hand-assemble layers versus using `build_world`, and demo 12 names its representative mantle pressure instead of a bare `1.0e9`. The RadialSolver benchmarks explain why they import the classic rheology objects (the input builders take that interface) rather than the `rheology_x` classes the demos teach. Demos 06/08/12 re-executed with committed outputs.
...
#### Documentation
* All future-structure (`_x`) documentation pages are now wired into the site navigation under a new `Future Structure` section; previously about 25 pages were orphaned and reachable only by direct URL. A new landing page (`future_structure.md`, first entry in the section) explains the rework, maps every classic module to its `_x` replacement, and walks through porting examples (world building and Love numbers, the standalone radial solver, rheology models). New group index pages collect the structures, tides, and utilities pages, and the `_x` demo and benchmark notebooks are grouped under the same `Future Structure` banner.

#### `TidalPy.RadialSolver_x`
* Added `homogeneous_love_numbers` (exposed at `TidalPy.RadialSolver_x`): a convenience helper that builds the radius, density, and moduli arrays for a single uniform solid layer and runs `radial_solver`. Extra keyword arguments pass through, so it drives both the shooting and propagation matrix paths. The demos and the performance suite previously each hand-rolled this block.
...
* `LayeredWorld.solve_love_numbers` / `solve_love_numbers_supplied` gained `solve_for` (replacing the boolean `solve_tidal`), selecting the surface boundary condition with the same names as the standalone `radial_solver`: `'tidal'` (default; tidal Love numbers k, h, l), `'loading'` (load Love numbers k', h', l'), or `'free'`. The world-level load Love numbers reproduce the standalone solver's on the same structure (`Tests/Test_Structures_x/Test_Worlds/test_world_love_loading_01.py`).
* `calc_3d_tides` can restrict the colatitude integral to a latitude band: with `latitude_summed`, pass `colatitude_min` / `colatitude_max` [rad] to integrate only that band (complementary bands add up to the full-sphere result to ~1e-10). A band narrower than the full sphere always uses the Gauss-Legendre quadrature (the analytic angular Gram table is full-sphere only); the band has no effect when colatitude is not summed. Tested in `Tests/Test_Structures_x/Test_Worlds/test_world_3d_tides_bands_01.py`.
* Layers now expose the same vectorized radius-query surface as the world: every real-valued profile getter (`get_density`, `get_gravity`, `get_pressure`, the post-melt and pre-melt moduli/viscosities) accepts a float or an `np.ndarray` of radii (array in, same-shape array out, evaluated in a nogil loop), and layers gained the `get_static_viscoelastics(radius)` / `get_state(radius)` shorthand bundles. `PhysicsLayer.calc_complex_shear_modulus` / `calc_complex_bulk_modulus` gained a radius-resolved form, `calc_complex_*(radius_m, frequency_rad_s)` (radius float or ndarray), applying the layer's rheology to the post-melt static modulus and viscosity stored at that radius by the world EOS solve, matching the world-level methods exactly; the one-argument layer-constant form `calc_complex_*(frequency_rad_s)` is unchanged. Tested in `Tests/Test_Structures_x/Test_Layers/test_layer_vectorized_01.py`.
* The System's binary serialization is complete for orbital evolution: the orbital rate engine (`c_OrbitSolver`) is stateless by design, so the serialized orbital elements, roles, and world records plus the documented reattach-after-load steps (each layer's material EOS model, the EOS profile via a re-solve, and the tide/spin/luminosity models; rheology/viscosity/partial-melt models serialize with the layers) fully reproduce `calc_world_evolution` after a `save_binary`/`load_binary` round trip (verified to machine precision in `Tests/Test_Structures_x/Test_System/test_system_03.py`).
