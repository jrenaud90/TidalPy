...
* A rebuilt tides engine (`TidalPy.Tides_x`): unsquared eccentricity functions for degrees 2..10 at truncation levels e^1..e^5, e^10, e^15, e^20, and unsquared obliquity functions (off, o^2, o^4, and fully general), used by one truncation-based Kaula potential engine in place of the classic per-scenario potential modules. Note the convention change: the classic eccentricity and inclination functions were the squared G^2/F^2, while the new ones are unsquared G/F.
