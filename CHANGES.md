...
* Added an on-demand 3D tidal stress/strain/heating kernel (`TidalPy.Tides_x.multilayer`). Rather than materializing a full `(radius × colatitude × longitude × time)` grid (the legacy `collapse_multilayer_modes` does, which blows up memory and cache), it computes the complex strain/stress tensors and the volumetric heating at a single point on demand by combining the radial solver's dense `y1..y6` at that radius with a 2D tidal potential. The strain kernel uses the exact Kervazo+2021-corrected forms with complex (viscoelastic) moduli at the mode frequency, a layer-type-dependent `dy1/dr`, and is restricted to solid layers (liquids contribute no shear dissipation). The complex moduli at radius are read through the radial solver's dense EOS path (`eos_call_si`). Validated to machine precision against the legacy instantaneous heating map (`tidal_heating_3d_point` / `_grid` in `heating_3d.py`; compiled kernel in `Tides_x.multilayer.stress_strain`).
  * The tidal potential is a proper class hierarchy `c_TidalPotentialBase : c_PhysicsBase` (`TidalPy.Tides_x.potential.tidal_potential`), following the same model pattern as the rheology/cooling/tide hierarchies: an enum, a `make_tidal_potential(name, config)` factory returning the rich subclass, binary serialization (class ids 1001-1002), and `calc_modes(...)` returning each active mode's signed frequency and the potential angular factor (`U` + its colatitude/longitude derivatives). Two truncations: `SyncLowEPotential` (alias `sync_low_e`) - synchronous rotation, low eccentricity, no obliquity (one mode); and `NSRModesPotential` (alias `nsr_modes`) - moderate eccentricity (`e^3`), non-synchronous rotation, no obliquity (the nine `l = 2` modes `n, 2n, 3n, 2o±kn`). Both reproduce the corresponding legacy `tides.potential` providers to machine precision.
  * Added the moderate-obliquity truncation `NSRMedObliquityPotential` (alias `nsr_modes_med_obliquity`; class id 1003) - moderate eccentricity (`e^3`), moderate obliquity (`obliquity^3`), non-synchronous rotation. It produces up to seventeen `l = 2` modes: the nine no-obliquity modes plus the obliquity-driven `m = 1` modes `o, o±kn` and the `m = 2` mode `2o` (adding the `P_21` associated Legendre harmonic). `calc_modes(...)` takes the planet `obliquity` (radians). It reproduces the legacy `tides.potential.nsr_modes_med_eccen_med_obliquity` provider (both with and without the static terms) to machine precision.
  * The TOML world builder now wires the tidal potential model onto a layered world automatically when it runs the `rheology` dissipation model, via the `[tides] tidal_potential_model` key (`sync_low_e` / `nsr_modes` / `nsr_modes_med_obliquity`; default `sync_low_e`, sourced from `config_x['tides']`). So a world built from config can use `get_3d_tidal_heating` without calling `set_tidal_potential_model` by hand. Analytic dissipation models (cpl/ctl/ctl_q) and layerless stars get no potential model (they have no depth-resolved solution).
  * The multi-mode on-demand path (`prepare_nsr_modes` + `tidal_heating_3d_point_nsr`) solves the radial response once per active mode frequency, then sums each mode's stress and strain tensors (each scaled by its own `freq_half = |omega|/2`) and computes the heating once from the combined tensors, preserving cross-mode terms exactly as the legacy collapse does. Validated to ~1e-12 (relative) against `collapse_multilayer_modes(use_modes=True)` for a non-synchronously rotating homogeneous body. Added `volumetric_heating(stress, strain)` to heat directly from mode-summed tensors.
  * **World-level, all-C++ 3D heating:** `LayeredWorld.get_3d_tidal_heating(orbital/spin state, radius, colatitude, longitude, time)` returns the on-demand 3D volumetric heating for a built world. The orchestration (loop the tidal potential's active modes, solve the world radial response once per mode frequency, sum each mode's freq_half-scaled stress/strain, heat once) lives on the rheology tide model (`c_RheologyTide::calc_3d_tidal_heating`), which the world delegates to; it calls the world's radial-solver/EOS members directly (no Python, no callbacks). Requires the rheology tide model (`set_tide_model`), a tidal potential model (`set_tidal_potential_model(make_tidal_potential(...))`), and a solved EOS; the analytic tide models are rejected (no depth-resolved solution). Reproduces the legacy `collapse_multilayer_modes(use_modes=True)` for a homogeneous Maxwell sphere to the radial-grid accuracy. The standalone array-driven `heating_3d.py` helpers remain for callers working from raw arrays rather than a world.
  * Pre-integrated angular tables, the analytic dimension collapse (orbit-average / latitude / longitude / radial sums), and obliquity modes are follow-on work.
