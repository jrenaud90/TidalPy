...
* 13 executed tutorial notebooks (`Demos_x/`), validation benchmarks against published Love numbers and BurnMan plus a performance-tracking harness (`Benchmarks_x/`), and end-to-end tests (`Tests/Test_E2E_x/`).
...
* Bundled solar-system worlds (`io`, `europa`, `luna`, `mercury`, `earth_simple`, `earth_prem`, `jupiter_simple`, `sol`, and the `sol_system` system): each states a temperature per layer and a viscosity law per solid layer, and each file's comments record which numbers were fitted to which observable (Io's heat output, the k2 of the liquid-core bodies, Luna's Q at the month and the year).
