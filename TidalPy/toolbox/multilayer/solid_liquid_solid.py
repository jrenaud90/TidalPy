...
@njit(cacheable=True)
def convergence_sls_static_liq(
...
@njit(cacheable=True)
def convergence_sls_dynamic_liq(
    tidal_y_solutions_by_layer: TidalYSolType, surface_solution: np.ndarray,
    gravity_array_layer1: np.ndarray, density_array_layer1: np.ndarray,
    radius_array_layer1: np.ndarray, orbital_freq: float
    ) -> np.ndarray:
    """ Determine the radial solution convergence for a planet with solid-liquid-solid structure.
...
def calculate_sls(
    ...
    interface_2_func = find_interface_func(
        lower_layer_is_solid=False, lower_layer_is_static=layer_1_static,
