...
from CyRK cimport cysolve_ivp, CySolveOutput, PreEvalFunc
...
cdef CySolveOutput solve_eos(
    ...
    while (pressure_diff > pressure_tol) and (iterations < max_iters):
        ...
        if iterations > 0:
            # Delete the old solution to remove a ref count
            del solution
        ...
        for layer_i in range(num_layers):
            ...
            memcpy(eos_input_ptr, eos_input_ptrs[layer_i], sizeof(EOS_ODEInput))
        ...
        surface_pressure_index = 2 * solution.size
        calculated_surf_pressure = solution[surface_pressure_index]
    ...
    if failed:
        printf("Warning in `solve_eos`: Integrator failed at iteration %d. Message: %s", iterations, solution.message_ptr)
    
    # Decrement old solutions ref count
    del solution
    ...
    # Step through each macro layer of the planet and determine final result
    cdef shared_ptr[GlobalEOSSolutionStorage] result = make_shared[GlobalEOSSolutionStorage]()
