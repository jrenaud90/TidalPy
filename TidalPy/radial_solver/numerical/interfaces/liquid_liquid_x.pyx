...
cdef void both_dynamic_x(double complex[:, :] lower_layer_y, double complex[:, :] upper_layer_y) nogil:
    ...
    """
    upper_layer_y[0] = lower_layer_y[0]
    upper_layer_y[1] = lower_layer_y[1]


cdef void static_dynamic_x(
    ...
    cdef double complex liquid_static_sol
    liquid_static_sol = lower_layer_y[0]
    ...
    for solution in range(2):
        ...
        if solution == 0:
            ...
            upper_layer_y[1] = -liquid_density * lower_layer_y[solution, 0]
            # y_5_dynamic = y_5_static
            upper_layer_y[2] = lower_layer_y[solution, 0]
            # y_6_dynamic = y_7_static + (4 pi G rho / g) y_5_static
            upper_layer_y[3] = lower_layer_y[solution, 1] + \
                             (4. * pi * G_to_use * liquid_density / interface_gravity) * lower_layer_y[solution, 0]
        else:
            # y_1_dynamic = 1.
            upper_layer_y[0] = 1.
            # y_2_dynamic = rho * g * y_1_dynamic
            upper_layer_y[1] = liquid_density * interface_gravity * solution_ys[solution, 0]
            # y_5_dynamic = 0.
            upper_layer_y[2] = 0.
            # y_6_dynamic = -4 pi G rho y_1_dynamic
            upper_layer_y[3] = -4. * pi * G_to_use * liquid_density * solution_ys[solution, 0]
