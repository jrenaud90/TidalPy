...
cdef void dy_solid_static_incompressible_x(
...
@cython.exceptval(check=False)
cdef void dy_liquid_static_incompressible_x(
    double radius, double[:] radial_functions, double[:] dy,
    double density, double gravity,
    unsigned int degree_l=2, double G_to_use=G) nogil:
    """ Calculates the radial derivative of the radial functions - for incompressible liquid layers.
