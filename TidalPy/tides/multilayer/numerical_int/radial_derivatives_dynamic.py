...
RadialFuncSolidType = Tuple[CmplxFltArray, CmplxFltArray, CmplxFltArray, CmplxFltArray, CmplxFltArray, CmplxFltArray]
RadialFuncLiquidDynamicType = Tuple[CmplxFltArray, CmplxFltArray, CmplxFltArray, CmplxFltArray]


# @njit(cacheable=True)
def radial_derivatives_solid_general(radius : FloatArray, radial_functions : RadialFuncSolidType,
                                     shear_modulus : CmplxFltArray, bulk_modulus : CmplxFltArray, density : FloatArray,
                                     gravity : FloatArray, frequency: FloatArray,
                                     order_l: int = 2) -> RadialFuncSolidType:
...
@njit(cacheable=True)
def radial_derivatives_solid_l2(radius: FloatArray, radial_functions: RadialFuncSolidType,
                                shear_modulus: CmplxFltArray, bulk_modulus: CmplxFltArray, density: FloatArray,
                                gravity: FloatArray, frequency: FloatArray) -> RadialFuncSolidType:
    """ Calculates the radial derivative of the radial functions in the most general form - for solid layers (l = 2).

    Allows for compressibility and dynamic tides.
    Tidal harmonic l must be = 2.
    ...
    dy1 = \
        y1 * (-2. * lame * lame_2mu) * r_inverse + \
        y2 * lame_2mu + \
        y3 * 6. * lame * lame_2mu * r_inverse

    dy2 = \
        y1 * (-dynamic_term * density + 12. * bulk_modulus * shear_modulus * lame_2mu * r2_inverse -
              4. * density * gravity * r_inverse) - \
        y2 * (4. * shear_modulus * lame_2mu * r_inverse) + \
        y3 * r_inverse * 6. * (density * gravity - 6. * bulk_modulus * shear_modulus * lame_2mu * r_inverse) + \
        y4 * 6. * r_inverse + \
        y5 * 3. * r_inverse * density - \
        y6 * density
    ...
    dy4 = \
        y1 * (density * gravity * r_inverse - 6. * bulk_modulus * shear_modulus * lame_2mu * r2_inverse) - \
        y2 * lame * lame_2mu * r_inverse + \
        y3 * (-dynamic_term * density + 24. * (lame + shear_modulus) * shear_modulus * lame_2mu * r2_inverse -
              2. * shear_modulus * r2_inverse) - \
        y4 * 3. * r_inverse - \
        y5 * density * r_inverse
    ...
    dy6 = \
        y1 * grav_term * 3. * r_inverse - \
        y3 * grav_term * 6. * r_inverse + \
        y6 * r_inverse
...
@njit(cacheable=True)
def radial_derivatives_liquid_l2(radius: FloatArray, radial_functions: RadialFuncLiquidDynamicType,
                                 bulk_modulus: CmplxFltArray, density: FloatArray,
                                 gravity: FloatArray, frequency: FloatArray) -> RadialFuncLiquidDynamicType:
    """ Calculates the radial derivative of the radial functions in the most general form - for liquid layers (l = 2).
    ...
    r_inverse = 1. / radius
    r2_inverse = r_inverse * r_inverse
    dynamic_grav = (1. - 2. * gravity * dynamic_term_inverse * r_inverse)
    grav_term = 4. * pi * G * density

    # See Eqs. 11--14 in KMN15
    # # dy2 and dy4 contain all three of: dynamic, viscoelastic, and gravitational terms.
    dy1 = \
        y1 * (-2. * r_inverse + 6. * gravity * r2_inverse * dynamic_term_inverse) + \
        y2 * ((1. / lame) - 6. * dynamic_term_inverse * r2_inverse / density) - \
        y5 * 6. * r2_inverse * dynamic_term_inverse

    dy2 = \
        y1 * (-dynamic_term * density - 4. * density * gravity * r_inverse +
              6. * density * gravity**2 * dynamic_term_inverse * r2_inverse) - \
        y2 * 6. * gravity * dynamic_term_inverse * r2_inverse + \
        y5 * 3. * density * r_inverse * dynamic_grav - \
        y6 * density
    ...
    dy6 = \
        y1 * 3. * grav_term * dynamic_grav * r_inverse + \
        y2 * grav_term * 6. * r2_inverse * dynamic_term_inverse / density + \
        y5 * grav_term * 6. * r2_inverse * dynamic_term_inverse + \
        y6 * r_inverse
