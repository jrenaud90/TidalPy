...
from .initial_solution_dynamic_incomp import (liquid_guess_kamata as liquid_dynamic_incomp_guess_kmn15,
                                              liquid_guess_takeuchi as liquid_dynamic_incomp_guess_ts72,
                                              solid_guess_kamata as solid_dynamic_incomp_guess_kmn15,
                                              solid_guess_takeuchi as solid_dynamic_incomp_guess_ts72)
...
from .initial_solution_static_incomp import (liquid_guess_saito as liquid_static_incomp_guess_s74,
                                             solid_guess_kamata as solid_static_incomp_guess_kmn15,
                                             solid_guess_takeuchi as solid_static_incomp_guess_ts72)
from .....utilities.performance import njit

# Known initial guess functions stored by: is_kamata, is_solid, is_dynamic
#    These are all compressible models.
...
def find_initial_guess(
    is_kamata: bool, is_solid: bool, is_dynamic: bool, is_incompressible: bool,
    ...
    if is_solid:
        if is_dynamic:
            if is_kamata:
                if is_incompressible:
                    result = solid_dynamic_incomp_guess_kmn15(
                        radius, shear_modulus, density, frequency,
                        order_l=order_l, G_to_use=G_to_use
                        )
                else:
                    result = solid_dynamic_guess_kmn15(
                        radius, shear_modulus, bulk_modulus, density, frequency,
                        order_l=order_l, G_to_use=G_to_use
                        )
            else:
                if is_incompressible:
                    result = solid_dynamic_incomp_guess_ts72(
                        radius, shear_modulus, density, frequency,
                        order_l=order_l, G_to_use=G_to_use
                        )
                else:
                    result = solid_dynamic_guess_ts72(
                        radius, shear_modulus, bulk_modulus, density, frequency,
                        order_l=order_l, G_to_use=G_to_use
                        )
        else:
            if is_kamata:
                if is_incompressible:
                    result = solid_static_incomp_guess_kmn15(
                        radius, shear_modulus, density,
                        order_l=order_l, G_to_use=G_to_use
                        )
                else:
                    result = solid_static_guess_kmn15(
                        radius, shear_modulus, bulk_modulus, density,
                        order_l=order_l, G_to_use=G_to_use
                        )
            else:
                if is_incompressible:
                    result = solid_static_incomp_guess_ts72(
                        radius, shear_modulus, density,
                        order_l=order_l, G_to_use=G_to_use
                        )
                else:
                    result = solid_static_guess_ts72(
                        radius, shear_modulus, bulk_modulus, density,
                        order_l=order_l, G_to_use=G_to_use
                        )
    else:
        if is_dynamic:
            if is_kamata:
                if is_incompressible:
                    result = liquid_dynamic_incomp_guess_kmn15(
                        radius, density, frequency,
                        order_l=order_l, G_to_use=G_to_use
                        )
                else:
                    result = liquid_dynamic_guess_kmn15(
                        radius, bulk_modulus, density, frequency,
                        order_l=order_l, G_to_use=G_to_use
                        )
            else:
                if is_incompressible:
                    result = liquid_dynamic_incomp_guess_ts72(
                        radius, density, frequency,
                        order_l=order_l, G_to_use=G_to_use
                        )
                else:
                    result = liquid_dynamic_guess_ts72(
                        radius, bulk_modulus, density, frequency,
                        order_l=order_l, G_to_use=G_to_use
                        )
        else:
            ...
            if is_kamata:
                if is_incompressible:
                    result = liquid_static_incomp_guess_s74(radius, order_l=order_l, G_to_use=G_to_use)
                else:
