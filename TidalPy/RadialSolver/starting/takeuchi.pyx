...
cdef void cf_takeuchi_solid_dynamic_incompressible(
        double frequency,
        double radius,
        double density,
        double complex shear_modulus,
    ...
    """ Calculate the starting guess at the bottom of a solid layer using assumptions:
        - Solid Layer
        - Dynamic Tides
        - Incompressible
    ...
    cdef double complex f_pos, f_neg
    f_pos = 0.0
    f_neg = -dynamic_term / gamma

    cdef double complex h_pos, h_neg
    h_pos = -1.0 * (degree_l_dbl + 1.0)
    h_neg = -1.0 * (1.0 + degree_l_dbl + (dynamic_term / gamma))
