...
cdef double complex cf_cinv(double complex z) noexcept nogil:
    ...
    if denom == 0.:
        ...
        inv = cf_build_dblcmplx(INF_DBL, NAN_DBL)
    else:
    ...
    if isfinite(z_real):
        if (z_real >= SCALED_CEXP_LOWER) and (z_real <= SCALED_CEXP_UPPER):
            ...
            if isfinite(z_imag):
                ...
                ret = cf_build_dblcmplx(NAN_DBL, copysign(NAN_DBL, z_imag))
    elif isnan(z_real):
        # z_real is NAN_DBL
        if z_imag == 0:
            ret = z
        else:
            ret = cf_build_dblcmplx(z_real, copysign(NAN_DBL, z_imag))
    else:
        # z_real is +- inf
        if z_real > 0:
            if z_imag == 0:
                ...
                # x = +inf, y = +-inf | NAN_DBL
                ret = cf_build_dblcmplx(z_real, NAN_DBL)
        else:
            if isfinite(z_imag):
                x = exp(z_real)
                c = cos(z_imag)
...
cdef double complex cf_clog(const double complex z) noexcept nogil:
    ...
    #  (returning -INF_DBL, signaling a floating-point exception, setting
...
cdef double complex cf_cpow(const double complex a, const double complex b) noexcept nogil:
    ...
    if b_real == 0. and b_imag == 0.:
        ...
        if b_real > 0.:
            ...
            # Here we should return a complex NAN_DBL
...
# Constants
########################################################################################################################
cdef double complex cmplx_NAN  = cf_build_dblcmplx(NAN_DBL, NAN_DBL)
