...
cdef extern from "intmap4_.cpp" nogil:
    ctypedef uint64_t KeyType4
    
    cdef KeyType4 convet_3key(int16_t l, int16_t m, int16_t p)

    cdef cppclass c_IntMap4:
        c_IntMap3() except +
        void reserve(size_t n)
        void clear()
        void set(int16_t l, int16_t m, int16_t p, int16_t q, double value)
        double get(cpp_bool& o_found, int16_t l, int16_t m, int16_t p, int16_t q) const
        size_t size() const


cdef class IntMap4:
    cdef c_IntMap4 intmap4_cinst
