""" Planet Iteration Package
    ...
    The goal of this package is to provide the user an easy, but less accurate, way to estiamte the interior structure
     of planets. It was designed with exoplanets in mind where some measurements (moment of inertia, etc) are completely
     unknown.
...
def planet_iterator(base_planet, goal_radius, goal_mass, ice_mass_frac: float = None,
    ...
    ----------
    base_planet :
        Instantiated planet object that acts as the starting point of the iteration
    goal_radius : float
    ...
    log(f'Iterating on planet: {base_planet.name}', level='info')

    # This is a two-layer iterator so a ice_mass_fraction is required. If none is provided then we will use whatever
    #  value was initially used in base_planet.
    num_layers = len(base_planet.layers)

    if num_layers < 2 or num_layers > 3:
        raise IncompatibleModelError('planet_iterator requires at least 2, and at most 3, layers in the base_planet.')
    if num_layers == 2:
        # No ice layer is used
        ice_mass_frac = None
    if num_layers == 3 and ice_mass_frac is None:
        ice_mass_frac = base_planet.layers[-1].mass / base_planet.mass
    ...
    new_config = copy.deepcopy(base_planet.config)
    new_config['radius'] = goal_radius
    bulk_density = goal_mass / ((4. / 3.) * np.pi * goal_radius**3)

    # Ge the names of the various layers
    core_layer_name = base_planet.layers[0].name
    mantle_layer_name = base_planet.layers[1].name
    if ice_mass_frac is not None:
        ice_layer_name = base_planet.layers[2].name
    else:
        ice_layer_name = None
    ...
    iterated_planet = base_planet
    iteration_step = 0
    while abs(goal_mass - iterated_planet.mass) / goal_mass > tolerance:
        iteration_step += 1
        log(f'\tIteration Step {iteration_step}')
        if iteration_step > 40:
            raise StopIteration('planet_iterator could not find convergence.')
        ...
        iterated_planet = build_from_planet(iterated_planet, new_config, new_name=base_planet.name)

    return iterated_planet
