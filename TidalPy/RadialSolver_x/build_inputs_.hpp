...
struct c_RadialSolverInputs {
    std::vector<double> radius_m;                                   // [m]      one entry per slice
    std::vector<double> density_kg_m3;                              // [kg/m3]  one entry per slice
    std::vector<std::complex<double>> complex_bulk_modulus_pa;      // [Pa]     one entry per slice
    std::vector<std::complex<double>> complex_shear_modulus_pa;     // [Pa]     one entry per slice
    std::vector<double> upper_radius_bylayer_m;                     // [m]      one entry per layer
    std::vector<std::size_t> slices_bylayer;                        //          one entry per layer
    double forcing_frequency_rad_s = TidalPyConstants::d_NAN;       // [rad/s]
    double planet_bulk_density_kg_m3 = TidalPyConstants::d_NAN;     // [kg/m3]
};
...
inline void c_fill_layer_complex_modulus(
        ...
        const std::vector<double>& static_modulus_pa,
        ...
        std::vector<std::complex<double>>& out)
{
    for (std::size_t slice_i = first; slice_i < first + count; ++slice_i) {
        out[slice_i] = model.calc_complex_modulus(
            static_modulus_pa[slice_i], viscosity_pas[slice_i], frequency_rad_s);
    }
}
        ...
        std::vector<double>& out_thickness_fraction_bylayer)
{
    ...
    out_thickness_fraction_bylayer.resize(num_layers);
    const double planet_radius3 = planet_radius_m * planet_radius_m * planet_radius_m;
    ...
    for (std::size_t layer_i = 0; layer_i < num_layers; ++layer_i) {
        ...
        out_thickness_fraction_bylayer[layer_i] = (layer_radius - radius_below) / planet_radius_m;
        ...
        c_RadialSolverInputs& out)
{
    ...
    if (!(planet_radius_m > 0.0) || !std::isfinite(planet_radius_m)) {
    ...
    out.radius_m.resize(total_slices);
    out.density_kg_m3.resize(total_slices);
    out.complex_bulk_modulus_pa.resize(total_slices);
    out.complex_shear_modulus_pa.resize(total_slices);
    out.upper_radius_bylayer_m.resize(num_layers);
    out.slices_bylayer = slices_bylayer;
    out.forcing_frequency_rad_s = forcing_frequency_rad_s;
    std::vector<double> static_shear_pa(total_slices);
    std::vector<double> static_bulk_pa(total_slices);
    std::vector<double> shear_viscosity_pas(total_slices);
    std::vector<double> bulk_viscosity_pas(total_slices);
    ...
    for (std::size_t layer_i = 0; layer_i < num_layers; ++layer_i) {
        ...
        const double layer_thickness = thickness_fraction_bylayer[layer_i] * planet_radius_m;
        ...
        out.upper_radius_bylayer_m[layer_i] = layer_radius;
        ...
        for (std::size_t slice_i = 0; slice_i < layer_slices; ++slice_i) {
            ...
            if (slice_i == 0) {
                out.radius_m[full_index] = last_layer_radius;
            } else if (slice_i == layer_slices - 1) {
                out.radius_m[full_index] = last_layer_radius + layer_thickness;
            } else {
                out.radius_m[full_index] = last_layer_radius + static_cast<double>(slice_i) * dr;
            }
            out.density_kg_m3[full_index] = layer_density;
            static_shear_pa[full_index] = static_shear_modulus_bylayer[layer_i];
            static_bulk_pa[full_index] = static_bulk_modulus_bylayer[layer_i];
            shear_viscosity_pas[full_index] = shear_viscosity_bylayer[layer_i];
            bulk_viscosity_pas[full_index] = bulk_viscosity_bylayer[layer_i];
        }

        detail::c_fill_layer_complex_modulus(
            *shear_rheology_bylayer[layer_i], static_shear_pa, shear_viscosity_pas,
            forcing_frequency_rad_s, first_slice_in_layer, layer_slices, out.complex_shear_modulus_pa);
        detail::c_fill_layer_complex_modulus(
            *bulk_rheology_bylayer[layer_i], static_bulk_pa, bulk_viscosity_pas,
            forcing_frequency_rad_s, first_slice_in_layer, layer_slices, out.complex_bulk_modulus_pa);

        first_slice_in_layer += layer_slices;
        last_layer_radius = layer_radius;
    }

    out.planet_bulk_density_kg_m3 = planet_bulk_density / planet_radius3;
}

// ---------------------------------------------------------------------------------------------------------------------
// c_build_rs_input_from_data
//
// Properties are given on an ascending user radius grid; `layer_upper_radius_bylayer_m` marks the
...
inline void c_build_rs_input_from_data(
        ...
        const std::vector<double>& static_bulk_modulus_pa,
        const std::vector<double>& static_shear_modulus_pa,
        const std::vector<double>& bulk_viscosity_pas,
        const std::vector<double>& shear_viscosity_pas,
        const std::vector<double>& layer_upper_radius_bylayer_m,
        const std::vector<const c_RheologyBase*>& shear_rheology_bylayer,
        const std::vector<const c_RheologyBase*>& bulk_rheology_bylayer,
        bool warnings,
        c_RadialSolverInputs& out)
{
    const std::size_t num_slices_input = radius_m.size();
    const std::size_t num_layers = layer_upper_radius_bylayer_m.size();
    ...
    const double planet_radius = radius_m[num_slices_input - 1];
    ...
    if (!c_isclose(layer_upper_radius_bylayer_m[num_layers - 1], planet_radius, 1.0e-9, 0.0)) {
        throw std::invalid_argument(
            ...
            ", found " + std::to_string(layer_upper_radius_bylayer_m[num_layers - 1]) + ".");
    }
    detail::c_check_rheology_vector(shear_rheology_bylayer, num_layers, "shear_rheology_model_tuple");
    detail::c_check_rheology_vector(bulk_rheology_bylayer, num_layers, "bulk_rheology_model_tuple");
    ...
    check_slice_vector(density_kg_m3, "density_array");
    check_slice_vector(static_bulk_modulus_pa, "static_bulk_modulus_array");
    check_slice_vector(static_shear_modulus_pa, "static_shear_modulus_array");
    check_slice_vector(bulk_viscosity_pas, "bulk_viscosity_array");
    check_slice_vector(shear_viscosity_pas, "shear_viscosity_array");
    for (std::size_t slice_i = 1; slice_i < num_slices_input; ++slice_i) {
        if (radius_m[slice_i - 1] > radius_m[slice_i]) {
    ...
    for (std::size_t layer_i = 1; layer_i < num_layers; ++layer_i) {
        if (layer_upper_radius_bylayer_m[layer_i] <= layer_upper_radius_bylayer_m[layer_i - 1]) {
            throw std::invalid_argument(
    ...
    out.upper_radius_bylayer_m = layer_upper_radius_bylayer_m;
    out.forcing_frequency_rad_s = forcing_frequency_rad_s;
    ...
    auto push_slice = [&](double radius, std::size_t source_index) {
        ...
        density_use.push_back(density_kg_m3[source_index]);
        shear_use.push_back(static_shear_modulus_pa[source_index]);
        bulk_use.push_back(static_bulk_modulus_pa[source_index]);
        shear_visc_use.push_back(shear_viscosity_pas[source_index]);
        bulk_visc_use.push_back(bulk_viscosity_pas[source_index]);
        const double r3 = radius * radius * radius;
        growing_mass += pi_4_3 * (r3 - last_r3) * density_kg_m3[source_index];
        last_r3 = r3;
    };
    ...
    for (std::size_t layer_i = 0; layer_i < num_layers; ++layer_i) {
        const double layer_upper_radius = layer_upper_radius_bylayer_m[layer_i];
        ...
        if (!c_isclose(radius_m[slice_i_input], last_layer_upper_radius, 1.0e-9, 0.0)) {
            if (warnings) {
                if (layer_i == 0) {
                    TIDALPY_LOG_WARN(
        ...
        while (slice_i_input < num_slices_input) {
            const double radius = radius_m[slice_i_input];
            if (c_isclose(radius, layer_upper_radius, 1.0e-9, 0.0)) {
                top_present = true;
            } else if (radius > layer_upper_radius) {
                break;  // First slice of the next layer.
            }
            push_slice(radius, slice_i_input);
            ++slice_i_input;
            if (top_present) {
                break;
            }
    ...
    out.radius_m = std::move(radius_use);
    out.density_kg_m3 = std::move(density_use);
    out.complex_bulk_modulus_pa.resize(total_slices);
    out.complex_shear_modulus_pa.resize(total_slices);
    out.planet_bulk_density_kg_m3 = growing_mass / (pi_4_3 * planet_radius * planet_radius * planet_radius);

    std::size_t first_slice_in_layer = 0;
    for (std::size_t layer_i = 0; layer_i < num_layers; ++layer_i) {
        const std::size_t layer_slices = out.slices_bylayer[layer_i];
        detail::c_fill_layer_complex_modulus(
            *shear_rheology_bylayer[layer_i], shear_use, shear_visc_use, forcing_frequency_rad_s,
            first_slice_in_layer, layer_slices, out.complex_shear_modulus_pa);
        detail::c_fill_layer_complex_modulus(
            *bulk_rheology_bylayer[layer_i], bulk_use, bulk_visc_use, forcing_frequency_rad_s,
            first_slice_in_layer, layer_slices, out.complex_bulk_modulus_pa);
        first_slice_in_layer += layer_slices;
    }
}
