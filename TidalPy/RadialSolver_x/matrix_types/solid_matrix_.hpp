...
/// complex shear modulus [Pa]. A null output is skipped, for a caller that reads only some of the three.
/// Incompressible body only.
    ...
    double G_to_use) noexcept
{
    ...
    for (size_t slice_i = first_slice_index; slice_i < num_radial_slices; ++slice_i)
    {
        ...
        if (fundamental_mtx_ptr)
        {
            ...
            fundamental_mtx_ptr[index_shift + 12] =
                (degree_l_dbl * rgp + 2.0 * l2mlm3 * complex_shear) * rl / (2.0 * dlp3);
            fundamental_mtx_ptr[index_shift + 13] =
                (rgp + 2.0 * (degree_l_dbl - 1.0) * complex_shear) * std::pow(radius, degree_l_dbl - 2.0);
            ...
            fundamental_mtx_ptr[index_shift + 19] =
                2.0 * (degree_l_dbl - 1.0) * complex_shear * std::pow(radius, degree_l_dbl - 2.0) / degree_l_dbl;
        ...
        if (inverse_fundamental_mtx_ptr)
        {
        ...
        if (derivative_mtx_ptr)
        {
            ...
            derivative_mtx_ptr[index_shift + 13] =
                -degree_l_dbl * lp1 * r_inv * (6.0 * complex_shear * r_inv - density * gravity);
            ...
            derivative_mtx_ptr[index_shift + 19] =
                2.0 * (2.0 * degree_l_dbl * degree_l_dbl + 2.0 * degree_l_dbl - 1.0) * complex_shear * (r_inv * r_inv);
